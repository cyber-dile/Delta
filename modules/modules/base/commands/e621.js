...
Command.lookup = (tags, callback) => {
    ...
    request.get({
        "method" : "GET",
        "uri": "https://e621.net/posts.json?tags=" + tags,
...
Command.display = async (interaction, id) => {
    if (Command.results[id]) {
        ...
        var embed = new MessageEmbed()
            .setTitle("e621 - " + tags + " " + artist)
            .setDescription("[Link to Original](https://e621.net/posts/" + post.id + ")")
...
Command.execute = async (interaction, data_override) => {
    ...
    var sd = await Delta.Data.Server.get(interaction.guild.id)
    if (rank < 3) {
        if (!interaction.channel.nsfw && !sd.all_channels_nsfw) {
            interaction.reply({content: "` You can't use this command in non-NSFW channels here! `", ephemeral: true})
            return
        }
    }
